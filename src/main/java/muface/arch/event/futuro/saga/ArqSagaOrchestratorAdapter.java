...
import muface.arch.event.ArqSagaStepInfo;
...
public class ArqSagaOrchestratorAdapter<T> implements ArqSagaOrchestratorPort<T> {
    ...
    @Autowired
    ArqSagaStepInputPort eventStoreConsumer;
    ...
    public ArqEvent startSaga(String sagaName, T data) {
        ...
        String typeEvent = "step-1";

        ArqEvent<?> dataEvent = new ArqEvent(applicationId, sagaName, typeEvent, sessionId, traceId,
                String.valueOf(transactionIdentifier), data);
    ...
    public void listen(ArqEvent<?> event) {
        ...
        if (event.getSagaStepInfo().isDoCompensateOp()) { // se ha ejecutado operación de compensación
            ...
            this.eventStoreConsumer.saveSagaStep(event.getArqContextInfo().getApplicationId(),
    ...
    private void actualizarEventoConCompensacion(ArqEvent event) {
        ...
        if (eventoTransaccion != null) {
            ...
            this.eventStoreConsumer.update(event.getArqContextInfo().getApplicationId(),
                    event.getSagaStepInfo().getSagaName(),
